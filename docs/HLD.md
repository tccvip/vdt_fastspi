...
**Single-stage flat ACL lookup (DD-15):**

Worker lcore áp dụng mô hình single-stage flat ACL lookup bên trong vòng xử lý của mình — một lần gọi `rte_acl_classify` duy nhất cho toàn bộ burst:
...
Không có Stage-2 lookup, không có lazy build, không có group context riêng biệt. Action (FORWARD/DROP) có sẵn trực tiếp sau một lần classify duy nhất.

**Batch ACL classify (DD-13):**

Worker lcore không thực hiện ACL lookup theo từng gói tin riêng lẻ. Thay vào đó, Worker thu thập toàn bộ batch mbuf từ ring, đọc metadata từ headroom của mỗi mbuf, build mảng keys[], rồi gọi `rte_acl_classify(flat_acl_ctx, keys[], results[], nb, 1)` một lần cho cả batch. Batch classify cho phép CPU prefetch và SIMD pipeline che giấu cache miss latency của ACL structure, cải thiện effective throughput đáng kể — đặc biệt quan trọng với 64-byte frames có budget per-packet rất nhỏ.
...
**Tra cứu action và dispatch:**

```
entry  = &flat_rule_table.rules[results[i] - 1]  /* O(1) */
...
Bước 12: Worker Dequeue và Single-Stage Flat ACL Classify
  ...
  - Single-stage: rte_acl_classify(flat_acl_ctx, keys[], results[], nb, 1)
    → results[i] = file_order + 1 của rule khớp có priority cao nhất.
  - Tra cứu O(1): entry = flat_rule_table.rules[results[i]-1]
    → action = entry->action (ACTION_FORWARD hoặc ACTION_DROP)
  - Per-worker hit counter được tăng cho group_idx tương ứng.
...
| Worker lcore | ACL classify với nhiều rule hoặc cache miss | Batch classify; single-stage flat ACL; tăng N_workers |
...
| DD-15 | Single-stage flat ACL lookup trong Worker | Một `rte_acl_ctx` duy nhất (`flat_acl_ctx`) chứa toàn bộ rule; Worker gọi `rte_acl_classify` một lần duy nhất mỗi burst; action có sẵn trực tiếp từ `flat_rule_table.rules[result-1].action` trong O(1) | Đơn giản hơn two-stage: không cần lazy build, không có race condition giữa Main và Worker, không có LRU eviction overhead. Với quy mô rule thực tế (vài chục đến vài trăm), flat context fit trong L2 cache. Critical path Worker chỉ có 1 ACL call thay vì 2. |
