...
|  Two-stage ACL classify   |
|    Stage-1: group lookup  |
|    Stage-2: filter lookup |
|  FORWARD → tx_ring        |
...
|---|---|---|
| NIC / PCAP PMD | — | Nguồn lưu lượng; cung cấp mbuf cho RX lcore |
| RX lcore | 1 | Poll burst từ NIC; lightweight hash để giữ pipeline balanced; enqueue mbuf pointer vào parser_ring |
...
| Worker lcore(s) | N | Batch two-stage ACL classify; áp dụng FORWARD/DROP; enqueue mbuf vào tx_ring (FORWARD) hoặc free mbuf (DROP) |
...
- Phát lệnh burst receive đến RX queue của port đã cấu hình. Burst size có thể cấu hình tại build time để cân bằng giữa độ trễ và hiệu quả CPU.
- Thực hiện **lightweight hash** trên từng mbuf ngay sau khi nhận: đọc IP src tại fixed byte offset trong frame (offset 26 với untagged Ethernet, offset 30 với VLAN tagged) mà không thực hiện parse đầy đủ header. Hash này phục vụ mục đích cân bằng tải sơ bộ và là placeholder cho khả năng scale sang nhiều parser lcore trong tương lai. Ở cấu hình hiện tại với 1 parser lcore, toàn bộ mbuf được enqueue vào parser_ring duy nhất.
- Enqueue mbuf pointer (không kèm metadata) vào parser_ring bằng `rte_ring_enqueue_burst()`. Ring chỉ truyền pointer 8 byte, không sao chép dữ liệu gói tin.
- Phát hiện kết thúc đầu vào: khi PMD `net_pcap` báo hiệu rằng tất cả frame PCAP đã được tiêu thụ (burst count trả về bằng không liên tiếp vượt ngưỡng EOI_THRESHOLD), thành phần này khởi tạo chuỗi tắt máy có trật tự cho toàn bộ pipeline.
- Duy trì bộ đếm rx_packets per-lcore phục vụ hạch toán thống kê.

RX lcore không thực hiện bất kỳ thao tác parse header, ACL lookup, hay quản lý bộ nhớ nào ngoài nhận và enqueue mbuf pointer.
...
Các rule đã phân tích được biên dịch vào cấu trúc phân loại ACL nội bộ `rte_acl_ctx`. Cấu trúc này là read-only sau khi build và được tất cả Worker lcore truy cập đồng thời mà không cần lock. Không hỗ trợ chỉnh sửa rule trong runtime.

**Two-stage ACL lookup (DD-15):**

Worker lcore áp dụng mô hình two-stage ACL lookup bên trong vòng xử lý của mình. Đây là tối ưu hóa nội bộ trong Worker, không phải giai đoạn pipeline riêng biệt:

- **Stage 1 — Group context lookup:** Tra cứu ACL context ở mức group để xác định gói tin thuộc group nào trong số các filter-group đã định nghĩa. Context Stage-1 nhỏ gọn (chứa rule đại diện cho từng group), fit trong L1/L2 cache của Worker lcore sau warm-up.
- **Stage 2 — Filter context lookup:** Mỗi group có một ACL context riêng biệt chứa tối đa 2048 filter, fit trong L1 cache khi đang active. Với tối đa 4096 group, Stage-2 context được quản lý theo cơ chế lazy build và LRU eviction — chỉ build context cho group đang active, không pre-build toàn bộ 4096 group cùng lúc (tránh sử dụng RAM quá lớn).

Cách tiếp cận two-stage tránh phải lookup một ACL context đơn khổng lồ chứa toàn bộ rule, giảm cache miss và cải thiện throughput per-core đáng kể ở quy mô rule lớn.

**Batch ACL classify (DD-13):**

Worker lcore không thực hiện ACL lookup theo từng gói tin riêng lẻ. Thay vào đó, Worker thu thập toàn bộ batch mbuf từ ring, đọc metadata từ headroom của mỗi mbuf, build mảng keys[], rồi gọi `rte_acl_classify(ctx, keys[], results[], nb, 1)` một lần cho cả batch. Batch classify cho phép CPU prefetch và SIMD pipeline che giấu cache miss latency của ACL structure, cải thiện effective throughput đáng kể — đặc biệt quan trọng với 64-byte frames có budget per-packet rất nhỏ.
...
Rule engine hỗ trợ tối đa 4096 filter-group, mỗi group chứa tối đa 2048 filter, tổng cộng tối đa 8.388.608 rule. Cấu hình vượt giới hạn này sẽ bị từ chối trong quá trình kiểm tra khi khởi động.
### 2.5 Thành Phần Filter Group (trong Worker lcore)

**Trách nhiệm:**

Thành phần Filter Group ánh xạ kết quả ACL classify (group_id và action) sang hành động xử lý gói tin phù hợp, thực hiện bên trong vòng xử lý của Worker lcore.
...
|---|---|
| FORWARD / ALLOW | mbuf được enqueue vào tx_ring bằng `rte_ring_enqueue()`. TX lcore sẽ drain tx_ring và truyền ra NIC TX. |
| DROP | mbuf được giải phóng ngay lập tức về mempool bằng `rte_pktmbuf_free()`. Drop counter được tăng lên. Gói tin không đến TX lcore. |

Mỗi gói tin nhận đúng một hành động. Rule mặc định đảm bảo không có gói tin nào thoát khỏi ACL classify mà không có hành động được gán (FR-023).
...
```
Bước 1: NIC RX / PCAP PMD
  - PMD đọc frame từ NIC hoặc file PCAP.
  - Một mbuf được cấp phát từ mempool và điền dữ liệu frame.
  - mbuf được đặt vào RX queue.

Bước 2: RX Burst Receive
  - RX lcore phát lệnh rte_eth_rx_burst() đến RX queue.
  - Một burst tối đa [BURST_SIZE] mbuf được trả về.
  - RX counter được tăng theo số lượng burst.

Bước 3: Lightweight Hash và Enqueue vào parser_ring
  - RX lcore đọc IP src tại fixed byte offset trong frame data.
  - Tính lightweight hash để phân phối vào parser_ring (hiện tại 1 parser).
...
Bước 12: Worker Dequeue và Batch ACL Classify
  - Worker lcore drain worker_ring bằng rte_ring_dequeue_burst().
  - Đọc pkt_meta_t từ headroom của từng mbuf trong batch.
  - Build mảng keys[] từ five-tuple metadata.
  - Stage 1: rte_acl_classify(stage1_ctx, keys[], group_ids[], nb, 1)
    → Xác định group_id cho từng gói tin.
  - Stage 2: rte_acl_classify(group_ctx[group_id], keys[], results[], nb, 1)
    → Xác định filter cụ thể và action trong group.
  - Per-worker hit counter được tăng cho group tương ứng.
...
Khi PMD `net_pcap` đã replay hết tất cả frame, `rte_eth_rx_burst()` tiếp tục trả về không mbuf. RX lcore phát hiện điều kiện này và khởi tạo quá trình tắt máy có trật tự theo thứ tự pipeline: RX lcore báo hiệu Parser lcore → Parser lcore drain xong rồi báo hiệu Worker lcore(s) → Worker lcore(s) drain xong rồi báo hiệu TX lcore → TX lcore drain tx_ring rồi thoát → Main lcore thu thập final stats, xuất tóm tắt phiên, flush và đóng file log trước khi ứng dụng thoát.
...
- **Flow affinity:** Cùng một flow (cùng five-tuple sau normalization) luôn được điều phối về cùng một Worker lcore. Worker đó sẽ warm cache ACL context Stage-2 của các group thường gặp trong flow set của mình, giảm cache miss khi classify.
...
|---|---|---|
| RX lcore | poll rate bị giới hạn bởi NIC throughput | Không cần can thiệp ở quy mô ≤ 40 Gbps |
| Parser lcore | Parse + hash dispatch single-threaded | Tối ưu prefetch; scale thêm Parser lcore trong tương lai nếu cần |
| Worker lcore | ACL classify với nhiều rule hoặc cache miss | Batch classify; two-stage ACL; tăng N_workers |
| TX lcore | MPSC dequeue + tx_burst serialization | TX lcore được giải phóng khỏi classify overhead; thường không là bottleneck ở ≤ 40 Gbps |
...
```
  ...
  - Build Stage-1 ACL context (group representative rules)
  - Build Stage-2 ACL context per group (filter rules within group)
  - Tất cả ACL context là read-only sau khi build
       |
       v
...
Mỗi rule đã được kiểm tra hợp lệ được dịch thành ACL entry nội bộ mã hóa điều kiện khớp theo từng trường và ưu tiên liên kết. Với kiến trúc two-stage:

- **Stage-1 ACL context** chứa một rule đại diện cho mỗi group — nhỏ gọn, fit trong cache.
- **Stage-2 ACL context** của mỗi group chứa toàn bộ filter rule thuộc group đó.

ACL engine sắp xếp các entry theo ưu tiên để tra cứu. Hành vi tie-breaking khi hai rule có cùng precedence được định nghĩa trong SDD.
...
| DD-06 | Cơ chế I/O gói tin | DPDK net_pcap PMD hoặc NIC vật lý | net_pcap PMD cho phép phát triển và xác nhận trên bất kỳ máy Linux nào mà không cần NIC vật lý. NIC vật lý được hỗ trợ cho môi trường production. Đáp ứng CON-04 và OBJ-01. |
...
| DD-13 | ACL classify tại Worker lcore với batch | Chuyển ACL từ RX/classifier lcore sang Worker lcore; batch rte_acl_classify(n=32) | ACL trên nhiều Worker lcore cho phép classify song song thay vì bottleneck trên 1 core. Batch classify với n=32 cho phép CPU prefetch và SIMD hide cache miss latency, cải thiện effective throughput từ ~10 Mpps lên ~30 Mpps per core so với single-packet lookup. |
...
| DD-15 | Two-stage ACL lookup trong Worker | Stage-1: group context lookup; Stage-2: filter context lookup — cả hai trong Worker lcore | Tránh một ACL context đơn khổng lồ không fit cache. Stage-1 nhỏ gọn fit L2 cache; Stage-2 per-group nhỏ fit L1 cache khi active. Tổng latency ~200 ns so với ~400–800 ns của monolithic lookup ở quy mô rule lớn. |
...
*Kết thúc tài liệu — SPIFAST-HLD-001-VI v1.3*
